...
public class LinkedHashMap<TKey, TValue> : IDictionary<TKey, TValue>
{
	...
	public bool Remove(TKey key)
	{
		...
		if (!dict.TryGetValue(key, out n)) return false;
		DoRemove(n);
		return true;
	}
	...
	{
		get { return list.Select(p => p.Key).ToArray(); }
	}

	public ICollection<TValue> Values
	{
		get { return list.Select(p => p.Value).ToArray(); }
	}
	...
	bool ICollection<KeyValuePair<TKey, TValue>>.Remove(KeyValuePair<TKey, TValue> item)
	{
		...
		if (!TryGetNode(item.Key, item.Value, out node)) return false;
		DoRemove(node);
		return true;
	}

	#endregion explicit ICollection implementation
}